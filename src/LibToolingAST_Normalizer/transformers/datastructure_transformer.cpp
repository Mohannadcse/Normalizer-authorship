        ...
        if (sm.isWrittenInMainFile(srcloc)) {
            ...
            for (DeclStmt::const_decl_iterator db = ds->decl_begin(), de = ds->decl_end(); db != de; ++db) {
                ...
                if (auto vardecl = dyn_cast<VarDecl>(vd)) {
                    ...
                    if (vardecl->getTypeSourceInfo()->getType().getAsString() == "int" || vardecl->getTypeSourceInfo()->getType().getAsString() == "short"
                                                                                                                                                   || vardecl->getTypeSourceInfo()->getType().getAsString() == "long"){
...
//                            ->getCanonicalTypeInternal().getAsString() << "\n";
    ...
    void replacecommand(Strategy strategy, int seed) {
        ...
        //this flag indicates if the normalizer hasn't been applied at all
        int transFlag = 0;
...
//                randomlySortVector<const DeclStmt*>(declstmtswithbooltype, seed);
            ...
            case int_to_longlong:
                randomlySortVector<const DeclStmt*>(declstmtswithinttype, seed);
                for(auto s : declstmtswithinttype){
                    if(transformXtoY(s, "long long")){
                        transFlag = 1;
                        continue;
                    }
                }
