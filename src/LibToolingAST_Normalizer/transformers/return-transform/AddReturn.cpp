...
void AddReturnMatchHandler::run(const MatchFinder::MatchResult &Result) {
  ...
  if (const auto *fn = Result.Nodes.getNodeAs<FunctionDecl>("intFunc")){
      if (!SM.isInMainFile(fn->getLocStart()))
          return;


      if (const auto CS = dyn_cast<CompoundStmt>(fn->getBody())) {
...
//  Finder.addMatcher(functionDecl(hasName("main")).bind("main"), &Handler);
  Finder.addMatcher(functionDecl(returns(isInteger())).bind("intFunc"), &Handler);
  Finder.matchAST(Ctx);
