...
//    }
    ...
    bool replacecommand(commandlineoption cmdoption){
        ...
        for (auto elem : allusabledeclarations){
//        llvm::outs() << "\n*************************\nELEM: " << elem->getNameAsString() << "\n";
        std::vector<NamedDecl*> from_decls;

        from_declname = elem->getNameAsString();
...
//        }
        into_declname = determineIntoVarName(elem);
        // Check that target name is not already used
        //        if(checkForNameConflict(into_declname))
        //            return false;
//        llvm::outs() << "BEFORE conflice: " << into_declname << "\n";
        checkForNameConflict(into_declname);
//        llvm::outs() << "After conflict: " << into_declname << "\n";
        /*** C. Rename all occurences of the declaration ***/
//        llvm::outs() << "***BEFORE Renaming :: from_decls:: "<< from_decls.size()<<"\n";
//        for(auto from_decl : from_decls){

            /*** C.1 Rename all occurences of a variable ***/
            if(auto from_vardecl = dyn_cast<VarDecl>(elem)){
                if(!renamevariabledecl(from_vardecl, into_declname))
                    return false;
            }
            /*** C.2 Rename all occurences of a fct ***/
            else if(auto from_fctdecl = dyn_cast<FunctionDecl>(elem)){
...
//        }
     ...
     */
    void checkForNameConflict(std::string &input_varname) {
        for(auto nameddecl : this->alldeclarationnames){
            if(nameddecl == input_varname){
//                llvm::errs() << "Conflict: Specified Name Already Exists (at least somewhere): "
//                             << input_varname << "\n";
                input_varname = input_varname + "_" +RenamingGuide::getRandomStringWith(3);
                alldeclarationnames.push_back(input_varname);
                //return true;
            }
        }
//        return false;
...
//            targetname = RenamingGuide::
//            (3) + typeasstring.substr(0,1) + "_Var";
            targetname = typeasstring + "_Var";
//            llvm::outs() << "determineIntoVarName:: Var Name:: " << targetname << "\n";
...
//            targetname = "do_" + typeasstring.substr(0,3) + RenamingGuide::getRandomStringWith(3) + "_fct";
            targetname = typeasstring + "_fct";
//            llvm::outs() << "determineIntoVarName:: FUnc Name:: " << targetname << "\n";
        }
        alldeclarationnames.push_back(targetname);
